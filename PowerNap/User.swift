...
class User: NSObject {
    ...
    class func addUser(useruid: String, firstName: String, lastName: String, email: String, age: String) {
        ...
        dbRef.child("users").child(useruid).observeSingleEvent(of: .value) { (snap: FIRDataSnapshot) in
            if snap.exists() {
                ...
                dbRef.child("users").child(useruid).setValue(["firstName": firstName, "lastName": lastName, "email": email, "age": age])
            }
        }
    }
}
